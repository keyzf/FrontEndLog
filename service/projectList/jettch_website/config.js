...
async function getStaticFileState() {
    ...
    var status = await page.open('https://www.jycash.cn/index.html');
    // 获取网页节点
    var content = await page.property('content');

    await instance.exit();
    // 判断网页是正常的依据目前是 索引在html 节点中是否包含 登录两个字，如果是就认定为正常，否则为不正常
    console.log(content.indexOf("home-page"));
    if (content.indexOf("home-page") != -1) {
        return 1;
    } else {
        return 0;
    }
...
module.exports.projectName = ' 吉帑业务官网';